...
from model.piece import AIRBORNE, CAPTURED, IDLE, LONG_REST, MOVING, SHORT_REST
...
def test_sprite_path_maps_idle_state_to_the_idle_folder():
    ...
    path = animator.sprite_path("p1", "KW", IDLE)
...
def test_sprite_path_maps_moving_state_to_the_move_folder():
    ...
    path = animator.sprite_path("p1", "KW", MOVING)
...
def test_sprite_path_maps_airborne_state_to_the_jump_folder():
    ...
    path = animator.sprite_path("p1", "KW", AIRBORNE)
...
def test_frame_advances_as_time_passes_in_a_looping_state():
    ...
    animator, clock = make_animator()
    animator.sprite_path("p1", "KW", IDLE)  # frame 1 at t=0
...
def test_frame_loops_back_to_one_after_a_full_cycle_in_a_looping_state():
    ...
    animator, clock = make_animator()
    animator.sprite_path("p1", "KW", IDLE)
...
def test_frame_freezes_on_the_last_frame_for_a_non_looping_state():
    ...
    animator, clock = make_animator()
    animator.sprite_path("p1", "KW", AIRBORNE)
...
def test_each_piece_id_tracks_its_own_animation_clock_independently():
    animator, clock = make_animator()
    animator.sprite_path("piece-a", "KW", IDLE)  # enters idle at t=0

    clock.now = 0.2  # 1 frame in for piece-a
    animator.sprite_path("piece-b", "KW", IDLE)  # piece-b enters idle now, at t=0.2

    path_a = animator.sprite_path("piece-a", "KW", IDLE)
    path_b = animator.sprite_path("piece-b", "KW", IDLE)
